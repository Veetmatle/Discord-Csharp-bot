...
public class BotService : IAsyncDisposable
{
    ...
    private async Task MigrateUserPuuidsAsync()
    {
        ...
        foreach (var entry in users)
        {
            ...
            try
            {
                ...
                var tftPuuid = await _riot.GetTftAccountPuuidAsync(account.gameName, account.tagLine, _shutdownCts.Token);
                if (tftPuuid != null)
                    _userRegistry.UpdateAccountTftPuuid(discordUserId, tftPuuid);
    ...
    private async Task SyncTftMatchIdsAsync()
    {
        ...
        foreach (var entry in users)
        {
            try
            {
                var tftMatchId = await _riot.GetLatestTftMatchIdAsync(entry.Value.TftPuuid ?? entry.Value.puuid, _shutdownCts.Token);
    ...
    private async Task RegisterRiotAccountAsync(SocketSlashCommand command, SocketSlashCommandDataOption subCommand)
    {
        ...
        if (account != null)
        {
            account.TftPuuid = await _riot.GetTftAccountPuuidAsync(nick, tag, _shutdownCts.Token);
            account.LastMatchId = await _riot.GetLatestMatchIdAsync(account.puuid, _shutdownCts.Token);
            account.LastTftMatchId = await _riot.GetLatestTftMatchIdAsync(account.TftPuuid ?? account.puuid, _shutdownCts.Token);
    ...
    private async Task CheckLatestTftMatchAsync(SocketSlashCommand command)
    {
        ...
        try
        {
            var tftPuuid = account.TftPuuid ?? account.puuid;
            string? matchId = await _riot.GetLatestTftMatchIdAsync(tftPuuid, _shutdownCts.Token);
            ...
            var me = matchData.info.participants.FirstOrDefault(p => p.puuid == tftPuuid);
    ...
    private async Task NotifyTftMatchFinishedAsync(RiotAccount account, TftMatchData matchData, CancellationToken ct = default)
    {
        ...
        var me = matchData.info.participants.FirstOrDefault(p => p.puuid == (account.TftPuuid ?? account.puuid));
        int placement = me?.placement ?? 0;
